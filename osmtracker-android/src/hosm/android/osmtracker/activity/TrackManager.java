...
import hosm.android.osmtracker.db.TracklistAdapter;
import hosm.android.osmtracker.exception.CreateTrackException;
import hosm.android.osmtracker.gpx.ExportToStorageTask;
...
public class TrackManager extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSharedPreferences = getApplicationContext().getSharedPreferences(LOG_TAG, 0);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (itemId == R.id.trackmgr_menu_newtrack) {
		...
		} else if (itemId == R.id.trackmgr_menu_continuetrack) {
			Intent i = new Intent(this, TrackLogger.class);
			i.putExtra(TrackLogger.STATE_IS_TRACKING, true);
			i.putExtra(Schema.COL_TRACK_ID, currentTrackId);
			startActivity(i);
		} else if (itemId == R.id.trackmgr_menu_stopcurrenttrack) {
			stopActiveTrack();
			edit.putLong("currentTrackId", -1).commit();
		} else if (itemId == R.id.trackmgr_menu_deletetracks) {
		...
		} else if (itemId == R.id.trackmgr_menu_exportall) {
			// Confirm
			new AlertDialog.Builder(this)
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		int itemId = item.getItemId();
		if (itemId == R.id.trackmgr_contextmenu_stop) {
			// stop the active track
			stopActiveTrack();
		} else if (itemId == R.id.trackmgr_contextmenu_resume) {
			...
			startActivity(i);
		} else if (itemId == R.id.trackmgr_contextmenu_delete) {
		...
		} else if (itemId == R.id.trackmgr_contextmenu_export) {
			new ExportToStorageTask(this, info.id).execute();
		} else if (itemId == R.id.trackmgr_contextmenu_osm_upload) {
			...
			i.putExtra(Schema.COL_TRACK_ID, info.id);
			startActivity(i);
		} else if (itemId == R.id.trackmgr_contextmenu_details) {
	...
	private void deleteTrack(long id) {
		if(id==mSharedPreferences.getLong("currentTrackId",-1)){
			edit.putLong("currentTrackId", -1).commit();
		}
